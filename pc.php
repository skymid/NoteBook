...
var max_row;	//数据库中的最大行号，用于新建记录后 赋值id
var wordcount;
var save_no=false;
var font_size=16;
...
$(function(){	
	...
	$("#title_").textbox('textbox').bind("blur", function () {
		$('#title_').textbox('textbox').css('background','#fff');
	}); 
	
	$("#title_").textbox('textbox').bind("focus", function () {
		$('#title_').textbox('textbox').css('background','#FFE5CC');
	});
		...
		columns:[[    
			...
			{field:'title',title:'标题',width:300,sortable:true,
				formatter:function(value,row){  
					...
					var content =  value ;
					if(row.link){
						content=value +' '+'<img src="images/link.png"/>'
					}
					  
					return content;  
				} 			
			},	
			{field:'set_lock',title:' ',width:90,sortable:true,
				formatter:function(value, row){
					if(row.link){
    ...
    $('#btn_add_nr').bind('click', function(){
		...
		$('#comb_type').combotree('setValue','');  //pid_id 单击菜单时传过来了
		$('#tb_TagBox').tagbox('setValues','');

		$('#tt_center').tabs('select',2);
		$('#title_').textbox('textbox').focus();
    });	
...
/*		tools:[{
	...
	$('#all_menu').tree({	
		...
		onCheck:function(node, checked){
			...
			//选择的node.id 没有包含在link时 或 取消选择时	
			//(主要是应付onLoadSuccess  中的  $('#all_menu').tree('check', node.target);)
			if(checked==false || row_nr.link.indexOf(node.id) < 0){
				$.ajax({							
					url:'update_link.php',
					type:'post',
					data:{
		...
		onLoadSuccess:function(node, data){
			if(row_nr.link){
				...
				{
					var node = $('#all_menu').tree('find',s[i] );
					$('#all_menu').tree('check', node.target);	
				}
				$('#all_menu').tree('scrollTo', node.target);
				$('#all_menu').tree('select', node.target);
			}		
		}             						
	})	
...
function edit_nr(data){
	...
	if(data){
		...
		$('#header_edit_title').html('setTitle',data.title+"   【 "+data.text+" 】");
		...
		//更新tab edit的面板的新标题	//保存了之后，就还原成'Edit'		
		$('#tt_center').tabs('update', {
			tab: $('#tt_center').tabs('getSelected'),
			options: {
				title: data.title,
...
function nr_save(){
	if(add_xg=='xg'){
		if($('#nr_id').textbox('getText').length==0){		//因为新建时没有 nr_id  (data.max_id  用这个就解决了)
			$.messager.show({
				title:'消息',
				msg:"新建内容，必须先保存一次。再重新打开再编辑！！",
				showType:'slide',
				style:{
					right:'',
					bottom:''
	...
	if($('#comb_type').combotree('getValue')!=''){
		nr_save_data = { 					
		...
		$.ajax({							
			...
			success:function(data){
			   ...
			   var data = eval('(' + data + ')'); 
			    ...
			    //新建记录后，再次保存用 或修改时任然用原来的id
				$('#nr_id').textbox('setText',data.max_id) 
		...
		$.messager.confirm('确定操作', '您确定要删除【'+row_nr.title+'】的记录吗？', function (flag) {
			if (flag) {			
				...
				$.ajax({
					...
					type : 'post',
					data : {
						nr_del : $('#dg').datagrid('getChecked')[0].nr,
					}
				});
				$('#dg').datagrid('reload');
				$('#tt_center').tabs('select',0);	
...
<body id="main_layout" class="easyui-layout">  
				...
				<div  class="easyui-panel" style="width:99%;height:99%;padding:15px;background:#fafafa;" data-options='border:false'>   
					...
					<div style="float:right">
						<div class="layui-btn-group">
							...
							<button class="layui-btn  layui-btn-primary" id="log">
			...
			<div data-options="region:'center'" >
				...
				<div id="tt_center" class="easyui-tabs" data-options="border:false,justified:false,tabWidth:250,narrow:true,pill:true,plain:true" style="width:100%;height:100%;">  
					...
					<div title="TOP" data-options="iconCls:'icon-lock'" style="padding:0px;display:none;">   
						<div id="p_nr_top" > 
							<div id="p_nr_top_" style="padding:15px;font-size:20px" ></div>
						</div>  
					</div>  
					...
					<div title="Edit" data-options="iconCls:'icon-page'" style="padding:0px;display:none;">   
						 <div id="note_edit" style="padding:5px">
    ...
    <div id="footer_top" style="padding:3px;">
        <div id="nr_footer_top" style="padding:4px;text-align:center" >top 123:123</div>
    </div>	
	
	<!--read foot-->	
    <div id="footer_read" style="padding:3px;">        
        <div class="m-toolbar">
            <div id="nr_footer" style="padding:4px;text-align:center" >read 123:123</div>           
            <div id="cc_link_div" class="m-right" style="padding:0 15px 0 0;">
				<input id="cc_link"  > 		
			</div>			
        </div>		
    </div>	
    ...
    <div id="nr_read_hh">
        <div class="m-toolbar">
            ...
            <div class="m-left">
				...
				<a class="easyui-linkbutton" data-options="iconCls:'icon-history',plain:true" onclick="alert('history')"></a>				
            </div>
			
			<div style="font-weight:bold;font-size:15px;color:#32CD32">
				<span class="m-title" id="nr_read_title">Custom Accordion Header</span>
			</div>
			<!--read head 右工具-->
            <div class="m-right">
	...
	<!--edit header-->
	<div  id="header_edit" style="padding:10px">	
		
		<div style="padding:0 10px 5px 10px">
			<input id="title_" class="easyui-textbox"  data-options="label: ' 标题：',labelWidth:50" style="width:93.5%"> 																 					
			<input id="nr_id" class="easyui-textbox" data-options="editable:false" style="width:5%" >
		</div>	
		
		<div style="padding:5px 10px 0 5px ;height:65px;width:99%">
			<div class="easyui-panel" style="padding:10px 10px 0 10px;background:#fafafa;" data-options="fit:true,border:true">	
				<a id="btn_save" onClick="tool_nr_save()" class="easyui-linkbutton" data-options="iconCls:'icon-save24',plain:true">Save</a> 
				<input id="fl">
				分类：<input id="comb_type" style="width:150px;" value="161"   > 										 										
				
				标签：<input id="tb_TagBox" type="text" style="width:45%"> 
				<input id="update_to_read" class="easyui-switchbutton" data-options="onText:'同步更新',offText:'不更新',width:90">
	...
	<!--edit footer-->
    <div id="footer_edit" style="padding:3px;">
        <div class="m-toolbar">
            <div class="m-title" id="nr_footer_edit">edit 123:123</div>
            <div class="m-left">
                <a onClick="nr_save()" class="easyui-linkbutton" data-options="iconCls:'icon-save24',plain:true">Save</a>
            </div>
            <div  id="cc_link2_div" class="m-right" style="padding:0 5px 0 0">
				<input id="cc_link2" > 		
			</div>			
        </div>
    </div>	
